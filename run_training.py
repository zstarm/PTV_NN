...
def main(save_every: int, total_epochs: int, batch_size: int, device: str, netinter: str, snapshot_path: str = "snapshot.pt",):
    NN_mm.ddp_setup(device_type=device.lower(), net_inter = netinter)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--netinter', default='', type=str, help='Network interface override')
    parser.add_argument('--batch_size', default=32, type=int, help='Input batch size on each device')
    args = parser.parse_args()
    main(args.save_every, args.total_epochs, args.batch_size, device=args.device, netinter=args.netinter)
